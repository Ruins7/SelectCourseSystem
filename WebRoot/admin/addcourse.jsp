...
-->
  ...
  课程名称：<s:textfield name="cou.courseName" id="1"/><br>
     ...
     开课单位：<s:select id="6" list="dep" listKey="depID" listValue="depName" name="cou.offerDepID"/><br/>  
     上课星期：<s:radio id="17" list="{'星期一','星期二','星期三','星期四','星期五'}" name="cou.weekday" value="'星期一'"/><br/>
     上课时间：<s:select id="7" list="{'08','10','14','16','18','20'}" name="timeForCourseHourB"/>: 
         <s:select id="8" list="{'00','20','45'}" name="timeForCourseMinB"/> ~ 
         <s:select id="9" list="{'09','11','15','17','19','21'}" name="timeForCourseHourE"/>:  
     ...
     所属培养计划：<s:select id="14" list="pla" listKey="planID" listValue="planName" name="cou.planID" value="1"/><br/>
     课程所属年级：<s:select id="15" list="{'一年级','二年级','三年级','四年级'}" name="cou.grade" value="'一年级'"/><br/>
     课程所属培养层次：<s:select id="16" list="{'本科','硕士','博士'}" name="cou.eduLevel" value="'本科'"/><br/><br>
     <s:submit value="添加课程"/><br>
  </s:form>
