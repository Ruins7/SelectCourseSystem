...
<html>
  <head>
		 <title>student send suggestion page</title>
  ...
  <div id="content">
     <div id="contentlift">
         <font color="red"> 
				...
				<br />
                <font color="red"><s:property value="message"/></font><!-- 提交失败！ -->
                <s:fielderror fieldName="errors"></s:fielderror>
    <s:form action="sendsugg.action" method="post">
                 反馈题目：<s:textfield id="1" name="sugg.title"/><br/>
                 反馈内容：<s:textarea id="2" name="sugg.suggContent" rows="15" cols="60"/>
              <input type="hidden" name="sugg.studentID" value="<s:property value="#session.studentID" />" />  
              <input type="hidden" name="sugg.studentName" value="<s:property value="#session.studentName" />" /> 
         <s:submit value="提交"/>
    </s:form>
     </div>
      <div id="contentright">
