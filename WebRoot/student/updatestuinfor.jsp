...
<html>
	<head>
		<title>updatestuinfor page</title>
				...
				<div id="contentlift">
					<font color="red"> <s:property value="noLogin" /> <!-- 您还木有登陆！请您登陆后操作(已拦截) -->
						<s:property value="loginNotFound" /> <!-- 用户名或密码错误 --> <s:property
							...
							value="notInTime" /> <s:property value="addfriendmessage" /> </font>
					<br />



					<font color="red"> <s:property value="haveBeenSign" />
						<!-- 修改失败！ --> </font> *为必填
					<br />
					<s:form action="updatestudent.action" method="post">
  ...
  QQ号码*：<s:textfield id="11" name="stu.qq" onblur="checknull(this)" />
     ...
     生日*： <s:textfield id="15" name="stu.birthday"
      ...
      培养层次*：<s:select id="16" list="{'本科','硕士','博士'}"
							name="stu.eduLevel" />
						<input type="hidden" name="stu.studentID"
							value="<s:property value= "sid" /> " />
					...
					<s:form action="fileupload.action" enctype="multipart/form-data"
						...
						<s:file name="upfile" label="选择上传文件"></s:file>
